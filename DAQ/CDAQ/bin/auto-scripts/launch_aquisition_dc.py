...
voltages = [32.7,33.7,34.7,35.7,36.7]		#Input voltage values before every run
temperature = "-170K"							#Input the temperature value before every run
...
for x in voltages: #Loop over all voltages
	...
	os.system("~/xenoscope/DAQ/CDAQ/bin/FBK-auto-dc.sh  %s" % (voltstrnamefile))	#os.system("~/path/bash.sh %s/d/.f" %variable) 
